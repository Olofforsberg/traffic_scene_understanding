...
{
    NegativeLogDataTermGrd(float dmax, float dmin, float sigma, float pOut, float pInv, const cv::Mat& ground_depth)
	{
        init(dmax, dmin, sigma, pOut, pInv, ground_depth);
	}

    inline float operator()(float d, int u, int v) const
	{
		if (d < 0.f)
			return 0.f;

        return std::min(nLogPUniform_, nLogPGaussian_.at<float>(u,v) + cquad_ * (d - fn_.at<float>(u,v)) * (d - fn_.at<float>(u,v)));
	}

	// pre-compute constant terms
    void init(float dmax, float dmin, float sigma, float pOut, float pInv, const cv::Mat& ground_depth)
	{
        //the distance to the ground plane from image vector point
        //lamda = d_plane*std::sqrt(u*u*fy*fy + v*v*fx*fx + fx*fx*fy*fy)/(a*u*fy+b*v*fx+fx*fy);
        //lamda-d_plane;

        // uniform distribution term, not used anymore
        nLogPUniform_ = logf(dmax - dmin) - logf(pOut);

		// Gaussian distribution term
        const int h = static_cast<int>(ground_depth.cols);
        const int w = static_cast<int>(ground_depth.rows);
        nLogPGaussian_ = cv::Mat::zeros(ground_depth.size(), CV_32F);
        fn_ = cv::Mat::zeros(ground_depth.size(), CV_32F);
        for (int u = 0; u < w; u++)
        {
            for (int v = 0; v < h; v++)
            {
                float fn = ground_depth.at<float>(u,v);

                float ANorm = 0.5f * (erff((dmax - fn) / (SQRT2 * sigma)) - erff((dmin - fn) / (SQRT2 * sigma)));
                nLogPGaussian_.at<float>(u,v) = logf(ANorm) + logf(sigma * sqrtf(2.f * PI)) - logf(1.f - pOut);
                fn_.at<float>(u,v) = fn;

            }
        }
        //std::cout << "fel soker 4" << std::endl;
